...
void peopleDelete() {
	...
	//std::ofstream outFire("afterIsChoose.json");
	//if (outFire.is_open()) {
	//	outFire << peopleList.toStyledString();
	//	outFire.close();
	//}
	
	ofstream outFire("afterIsChoose.json", std::ios::binary);
	if (outFire.is_open()) {
		Json::StreamWriterBuilder builder;
		builder.settings_["emitUTF8"] = true;
		std::unique_ptr<Json::StreamWriter> writer(builder.newStreamWriter());
		writer->write(peopleList, &outFire);
		outFire.close();
	}
	...
	while (1)
	{
		...
		if (msg.lbutton == 1) {// 判断鼠标左键是否按下
			...
			if (x >= 0 && x < 245 && y > 150 && y < 178) {// 点击返回目录按钮
				...
				inputOutput(name, drawAddPeopleWindow, 295, 205, 662, 242);
			}
			else if (x >= 295 && x < 662 && y > 254 && y < 291) {// 点击性别输入框
				inputOutput(sex, drawAddPeopleWindow, 295, 254, 662, 291);
			}
			else if (x >= 295 && x < 662 && y > 303 && y < 340) {// 点击手机号码输入框
				inputOutput(PhoneNum, drawAddPeopleWindow, 295, 303, 662, 340);
			}
			else if (x >= 295 && x < 662 && y > 352 && y < 389) {// 点击身份证号码输入框
				inputOutput(IdentityCardNum, drawAddPeopleWindow, 295, 352, 662, 389);
			}
			else if (x >= 295 && x < 662 && y > 402 && y < 439) {// 点击地址输入框
				inputOutput(address, drawAddPeopleWindow, 295, 402, 662, 439);
			}
			else if (x >= 295 && x < 662 && y > 452 && y < 489) {// 点击邮箱输入框
				inputOutput(mailbox, drawAddPeopleWindow, 295, 452, 662, 489);
			}
