...
void drawTicketMSGWindow() {
	...
	for (unsigned int i = 0; i < people_ticketMsg["man"].size(); i++) {// 循环渲染数据
		...
		textCenter(people_ticketMsg["endPlace"].asString().c_str(), 249, 279 + i * 87, 87, 92);
...
void ticketMSGWindow() {
	...
	if (jumpInterfact == 8)// 查看电子票据
	{
		ticketShow();
	}
	if (jumpInterfact == 2)// 查看电子票据
	{
		directoryWindow();
	}
}
...
void drawTicketShow() {
	...
	textCenter("DG", 167,102,78,153);// 出发地
	textCenter(people_ticketMsg["endPlace"].asString().c_str(), 627, 102, 78, 200);// 到达地
